...
class TestProjects(TestBase):
    def test_create_project(self):
        ...
        projects = self.request(
            "get", "/api/v1/{0}/projects".format(self.org1), {}, token, 200
        )
    ...
    def test_project_access(self):
        ...
        project2 = self.request(
            "post", "/api/v1/{0}/projects".format(self.org2), payload2, token2, 201
        )
        # list user1 projects
        projects = self.request(
        ...
        self.request("get", "/api/v1/{0}/projects".format(self.org2), {}, token1, 403)
        # access user2 project by its id from user1 token
        self.request(
            "get",
            "/api/v1/{0}/projects/{1}".format(self.org2, project2["id"]),
            {},
            token1,
            403,
        )
    ...
    def test_project_details(self):
        ...
        project_details = self.request(
            "get",
            "/api/v1/{0}/projects/{1}".format(self.org1, project["id"]),
            payload,
            token,
            200,
        )
